...
For transitively anchored transactions, it requests additional chain data to determine whether the transaction has a direct anchor.
...
The loop is intentionally simple. Whenever the task returns `TaskProgress::Query(heights)`, the driver resolves those heights and continues polling.
...
This design supports synchronous or asynchronous drivers and provides independence from a particular source or runtime. A task does not care where blocks come from or how the driver obtains them.
...
`BlockQueries` acts as a cache for a task. It deduplicates requests, tracks which heights have been fetched, and allows fetched data to be reused.
